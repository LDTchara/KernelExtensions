...
{
    /// <summary>
    /// 运行时临时**添加**一条节点链接（只改 links，不写 org 基线）。
    ///
    /// XML 用法：
    ...
    /// </summary>
    public class LinkControlAddAction : DelayablePathfinderAction
    {
        [XMLStorage] public string SourceComp;
        [XMLStorage] public string TargetComp;

        public override void Trigger(OS os) => OrgLinksStorage.Modify(os, SourceComp, TargetComp, add: true);
    }
}
