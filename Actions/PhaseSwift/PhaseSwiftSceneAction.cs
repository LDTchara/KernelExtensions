...
namespace KernelExtensions.Actions.PhaseSwift
{
    public class PhaseSwiftSceneAction : DelayablePathfinderAction
    {
        ...
        [XMLStorage] public int MusicPhase = -1;
        ...
        public override void Trigger(OS os)
        {
            if (PhaseSwiftManager.IsInitialized)
            {
                ...
                if (MusicPhase >= 0)
                    PhaseSwiftManager.SwitchMusicPhase(MusicPhase);
                return;
            }
            ...
            if (exe != null)
            {
                ...
                if (MusicPhase >= 0)
                    exe.SwitchMusicPhase(MusicPhase);
            }
        }
        ...
        public override void LoadFromXml(ElementInfo info)
        {
            ...
            if (info.Attributes.TryGetValue("Delay", out string delayStr)) Delay = delayStr;
            if (info.Attributes.TryGetValue("DelayHost", out string delayHost)) DelayHost = delayHost;
        }
    }
}