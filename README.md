...
在试炼配置的 `<OnPhaseStart file="..."/>`、`<OnComplete file="..."/>` 或 `<OnFail file="..."/>` 中，可以指定一个 Action 的 XML 文件。动作文件支持使用 `Delay` 属性（单位：秒）来控制执行顺序，**数值越小越先执行，负数表示在过去的时间执行（即立即执行但时间戳会回溯）**。例如：

```xml
<ConditionalActions>
  <OnConnect target="TMSDHS">
    <!-- Delay 负值：立即执行，但游戏内时间戳会显示为过去 -->
...
- **动作文件中的延迟**：`Delay` 属性数值越小越先执行。负值表示在过去的时间执行（消息会显示为较早发出）。需要 `DelayHost` 的动作必须指定一个有效的节点（如 `playerComp` 或任意拥有 `FastActionHost` 的节点）。
...
In your trial config's `<OnPhaseStart file="..."/>`, `<OnComplete file="..."/>` or `<OnFail file="..."/>`, specify an action XML file. Actions support the `Delay` attribute (in seconds). **Smaller delay values execute first; negative delays execute in the past (immediate but with backdated timestamps).** Example:

```xml
<ConditionalActions>
  <OnConnect target="TMSDHS">
    <!-- Negative delays: execute immediately but appear with past timestamps -->
    ...
    <!-- Positive delays: executed in order from smallest to largest -->
    <ChangeAlertIcon Target="TMSDHS" Type="irchub" DelayHost="Cheat" Delay="1.0"/>
...
**Important**: Only actions that support the `Delay` attribute (e.g., `AddIRCMessage`, `RunFunction`, `ChangeAlertIcon`, `AddConditionalActions`) can be delayed. Non‑delayable actions execute immediately.
...
<TrialConfig>
  ...
  <!-- Action executed at trial start (optional) -->
  <OnStart file="Actions/TrialStart.xml" />
...
- **Delay in actions**: Smaller `Delay` values execute first. Negative values execute in the past (messages appear with earlier timestamps). Actions that require a `DelayHost` must specify a valid node (e.g., `playerComp` or any node with `FastActionHost`).
