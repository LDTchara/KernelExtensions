...
!!! warning "`%` 标题行与横幅标题不支持非 ASCII 字符"
    这几处使用游戏的标题字体（`Kremlin`）——官方**未提供本地化版本**（中文环境下也无中文字形），因此中文等非 ASCII 字符会显示为 `?`：

    - 报幕名单中以 `%` 开头的**标题行**
    - 横幅（ShowTitle）的**标题**文字

    **其余文本不受影响**：报幕正文与 `^` / `$` 行、横幅正文、演讲文本（`SpeechTextFile`）在中文环境下都会使用官方中文字体（`zh-cn_FontXX`），中文可正常显示。
