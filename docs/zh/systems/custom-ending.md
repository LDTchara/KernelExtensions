...
!!! note "AfterMusic 的切换行为"
    ...
    **做过两段式淡出后，切歌改为「立刻切换」**（`playSongImmediatley`），不再叠加 `transitionToSong` 的淡出——报幕曲此时已渐弱到静音，再叠一层淡出会出现「静音后突然又响一下再淡出」的割裂感。
...
（表内 `A` / `B` 代表任意曲目路径，如 `Music/Smile,please smile pt2.ogg`）

| 你想要的结局音乐行为 | `OnCreditMusic` | `AfterMusic` | `CreditsFadeOutTime` |
|---------------------|-----------------|--------------|----------------------|
| 报幕放 A，结束后**继续放 A**（不断、不淡出） | `A` | **留空 / `NONE`** | 任意（不生效） |
| 报幕放 A，A 渐弱到静音后**立刻切到 B** | `A` | `B` | `> 0`（默认 5） |
| 报幕放 A，结束后**从头重播 A** | `A` | `A` | 任意 |
| 报幕用原版曲，结束后继续放 | 留空 | 留空 / `NONE` | 任意 |

!!! warning "三个容易踩的点"
    **① 「继续播」不能靠填同名。** 把 `AfterMusic` 填成与 `OnCreditMusic` **相同**的曲子，得到的是**从头重播**，不是接着播——因为 `transitionToSong` 在同名时会直接跳过，这种情况改走 `playSongImmediatley`（重播）。想让音乐天然延续，**必须留空 / `NONE`**。

    **② 「不切换」与「不淡出」是同一件事。** 两段式淡出以「`AfterMusic` 非空」为前提，所以留空时**根本不会淡出**，`CreditsFadeOutTime` 填什么都不生效。

    **③ 反过来做不到「淡出但不切歌」。** 淡出必然伴随切换：`AfterMusic` 有值就会切，留空就不淡出。想要「渐弱到静音后就静着、不回游戏音乐」，目前没有开关支持。

    **另附**：`OnCreditMusic` 留空 = 用**原版** `Music\Bit(Ending)`，**不是「不播」**。
