...
!!! warning "报幕与横幅不支持非 ASCII 字符（中文/日文会变 `?`）"
    报幕名单与横幅文本在绘制前经过原版字符清洗（ASCII 白名单），**中文、日文等非 ASCII 字符会显示为 `?`**。  
    演讲文本（`SpeechTextFile`）**不受此限制**，可正常显示中文（取决于所用字体）。这是已知限制——报幕与横幅区域请避开非 ASCII 字符。
