...
- **读档后飞机状态不可靠**：飞机 daemon 的运行时状态（坠落进度、`IsCrashed` 终结标志等）不随存档
  完整保存。读档后节点本身能恢复正常，但飞机的位置与运行状态可能不正确。

  建议在剧情设计上**避开「飞机飞行途中存读档」的区段**。若确实需要在关键区段阻止存档，
  可依赖第三方模组提供的能力（如 Stuxnet 的 `DenySaves`）——KE 自身不提供存档拦截。
