...
KernelExtensions 目前提供以下可配置系统，多数通过 Flag 或自定义 Action 触发，由 XML 配置文件驱动：
...
- **相位穿梭（PhaseSwift）**：多场景拓扑 + 多轨音乐的场景切换系统。
- **定时器（Clock）**：按固定间隔循环执行一组 Action 的通用定时器。
- **节点图标（SetNodeIcon）**：静态声明或运行时切换任意节点的图标。
- **自定义结局（StartEnding）**：报幕 / 演讲 / 制作人员名单的可配置结局流程。
- **标题横幅与全屏警告（ShowTitle / ScreenBleedWCC）**：剧情提示与系统告警的全屏表现。
...
各系统的触发大多依赖 Hacknet 原生的 Flag 机制（或自定义 Action），一般情况下 Flag 只能通过 Action 系统添加/移除，不能使用控制台指令直接操作。
